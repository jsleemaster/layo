...
| Layer model | Pages, frames, shapes, text, images, groups, sections, parent/child hierarchy, selection. | Pages, frames, rectangles, text, images in type model, nested children. No groups/sections UI. | Adopt. Add group/ungroup, layer reorder, layer nesting, and image insertion. |
| Geometry | Move, resize, rotate, flip, x/y/w/h fields, bounding boxes. | Move, bottom-right resize, x/y/w/h fields, rotation in model only. | Adopt. Add all resize handles, rotation, flip, proportional scale, and keyboard nudging. |
| Precision editing | Alignment, distribute, tidy, rulers, guides, layout guides. | Not implemented. | Adopt after layout foundation. |
...
| Fills and imagery | Solid fills, gradients, image fills, patterns, color picker. | Solid fill only. Image node type exists without UI asset flow. | Adopt. Gradients and images are high value; patterns later. |
