...
- `Shift+1` is a semantic fit command, not a manual zoom step. It may zoom below the ordinary 25% control floor, down to 1% when necessary, so the complete selected bounds remain inside the padded canvas viewport; panning must preserve that fitted scale.
