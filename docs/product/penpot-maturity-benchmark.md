...
points are inert or centrally rejected, and stale version/file responses cannot
replace the latest requested preview. Restore also publishes the recovered
document through the active Yjs session before another local edit.

Independent review expanded the product gate beyond the visible preview. Every
same-file document write and version mutation now shares one ordered browser
queue; complete snapshots and Yjs preserve all top-level design-system fields;
Yjs Undo/Redo tracks local transactions without replacing later remote edits;
and cancelled uploads are removed only after the server confirms no current
document, component, or saved version references the asset. HTTP, browser, and
MCP E2E processes also share one marker-owned temporary storage root.

The final reliability review added base-aware three-way complete-snapshot merge,
explicit divergent-order conflicts, per-ID Yjs design-system collections with
same-ID deduplication and document-version retention, a synchronous Restore
mutation barrier, serialized asset-reference cleanup including component
variants, explicit MCP subprocess environment allowlisting, and project/version
request generations. These are team-product data-loss and isolation boundaries,
not preview-only polish.

Focused re-review then closed three residual races: stale reorders now conflict
when they cannot preserve a concurrent insertion's placement, current-project
IndexedDB writes are serialized so the latest selection persists last, and both
successful and rejected stale project fetches are prevented from replacing active
state or status.

Exact-head review found that an already-started stale IndexedDB write still
needed compensation when the newer request failed before persistence. The
...
`docs/product/penpot-file-version-visual-preview-delta.md`.
...
2. Full comment and history workflow beyond selected-node create/list/resolve/reply, viewport unresolved-comment bubbles, persisted mentions, structured local team-member mention targets, local unread/read state, local project/file unread summaries, local viewer-targeted mention notification counts, browser polling fallback, shared-filesystem durable SSE comment event replay with EventSource cursors, retained local create/reply/resolve activity feed, manual/automatic saved file versions, current-file diff plus complete read-only canvas preview, recovery-safe server/Yjs restore path, pinned saved-version checkpoints, manual saved-version delete, and explicit unpinned retention cleanup: full CRDT-backed live comment editing, hosted durable pub/sub beyond shared-filesystem event logs, external notification channels, multi-page preview navigation, branch/review/merge, and richer recovery workflow.
