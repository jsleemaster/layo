...
- Figma REST/MCP import paths for real Figma files, where credentials are provided by the user. The first no-write preflight is in place, opaque `.fig` binaries still point users to REST export, and Figma REST JSON can now create a fresh Layo project for basic pages, frames, rectangles, and text.
...
7. Keep the Penpot/Figma migration preflight and first Figma REST JSON write import green, then extend full Figma/Penpot mapping on top of the local asset pipeline with exported image assets, components, variants, effects, and Penpot ZIP shape mapping.
