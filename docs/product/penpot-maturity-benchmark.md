...
2. Full comment workflow beyond selected-node create/list/resolve/reply, viewport unresolved-comment bubbles, persisted mentions, local unread/read state, local project/file unread summaries, and retained local create/reply/resolve activity feed: live comment sync, real team activity notifications, real team-member mention targeting, full visual diff, branch/review/merge, pinned/version preview controls, retention controls, and richer recovery workflow beyond the landed manual/automatic saved file versions, basic diff preview, and restore path.
