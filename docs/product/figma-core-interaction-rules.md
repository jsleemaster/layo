...
| Text editing | Double-clicking a text layer enters direct text edit mode on the canvas. | Text layers open an inline canvas textarea on double-click and keep inspector text in sync while editing. | Adopted for text layers. Rich text runs and nested text drill-down remain later. |
| Multi-selection | Shift-click adds/removes canvas objects. Marquee selects objects in an area. Cmd/Ctrl marquee can select nested layers. | Shift-click add/remove and drag area selection use `nodeIds`; `nodeId` remains the primary selected node. Multi-selected layers now show one combined group outline and combined dimensions. | Adopted for Shift-click, 영역 선택, group outline, and combined size readout. Cmd/Ctrl nested marquee remains deferred. |
...
| Fit selection | Shift+1 centers the complete selected bounds with viewport padding, even when a large board requires a zoom below the ordinary 25% manual floor. | Selection fit supports a fit-only floor down to 1%; pan preserves the fitted zoom and subsequent zoom input remains bounded. | Adopted. Keep oversized-frame geometry and browser coverage green. |
...
| Resize | Objects resize through handles; directional cursors indicate the active resize axis; Shift preserves aspect ratio; constraints can be ignored while resizing with platform command modifier. | Four corner handles, corner-specific resize cursors, and an immediate selection size badge are implemented for a single selected layer. Edge handles are intentionally excluded. Multi-selection shows a group outline and combined size without transform handles. | Adopted first slices. Aspect-ratio resize, rotation, multi-selected bounding-box resize behavior, and transform modifiers remain later. |
| Alignment/distribution | Align controls align selected layers; distribute requires multiple selected layers and keeps outer objects fixed; tidy up adds stricter row/column/grid logic. | Inspector alignment and distribution controls operate on `selection.nodeIds` through one batch geometry command. Align/distribute controls are grouped, tooltip-labeled, and expose disabled distribution state for single-selection. Selection-specific toolbar duplicates were removed. | Adopted for selected-layer alignment/distribution and Inspector control grouping. Tidy up remains deferred. |
...
5. Keep multi-selection group outline and combined size readout green before adding multi-selected bounding-box resize.
6. Extend the landed live snap/measurement and resize slices with rotation, rulers, manual guides, and snap settings.
7. Extend copy/paste with multi-node selection and paste-here after the single-object session clipboard stays green.
...
- Rotate, flip, aspect-ratio resize, and multi-selected bounding-box resize behavior.
