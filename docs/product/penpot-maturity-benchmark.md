...
| Import/export | Penpot file import/export, backups, shared libraries, assets. | Local design JSON, asset storage, image paste/drop, code export, and document-local DTCG color/spacing token import/export. | Open project export/import, Penpot/Figma migration paths, asset packaging, and backup/restore. |
...
2. Full comment workflow beyond selected-node create/list/resolve/reply, viewport unresolved-comment bubbles, persisted mentions, structured local team-member mention targets, local unread/read state, local project/file unread summaries, and retained local create/reply/resolve activity feed: live comment sync, real team activity notifications, full visual diff, branch/review/merge, pinned/version preview controls, retention controls, and richer recovery workflow beyond the landed manual/automatic saved file versions, basic diff preview, and restore path.
3. Design-system maturity beyond first-class color/spacing tokens, fill bindings, layout gap/padding spacing-token bindings, and DTCG color/spacing import/export: token sets/modes, typography tokens, reusable styles, variants, shared libraries, and code component mappings.
