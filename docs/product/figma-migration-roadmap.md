...
- No-write external migration preflight for Penpot ZIP/.penpot, Figma REST JSON, and opaque Figma binary files through server review, CLI, web API helpers, and the file panel. This detects source type, archive kind, document candidates, asset candidates, blockers, and next steps before any storage write.
...
- Figma REST/MCP import paths for real Figma files, where credentials are provided by the user. The first no-write preflight is in place for Figma REST JSON and blocks opaque `.fig` binaries with a concrete REST export next step.
...
7. Keep the Penpot/Figma no-write migration preflight green, then build full Figma/Penpot import mapping separately on top of the local asset pipeline, starting with frames, rectangles, text, and exported image assets before variants or advanced effects.
