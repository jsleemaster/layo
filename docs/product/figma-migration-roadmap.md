...
- Browser editor shell with a Figma-like left mode rail, default asset-library panel, top file tabs, rulers, bottom floating toolbar, right Inspector action strip, expanded frame preset categories, creation, selection, dragging, corner-only resizing, direct double-click text editing, selection size badges, inspector geometry, grouped Inspector alignment/distribution controls, expanded object context-menu selection/flip/fit/export actions, color/text editing, undo/redo shortcuts, zoom, hover measurement overlays, selected-frame padding/child-spacing guides, and a multi-selection group outline with combined dimensions.
