...
- Treat Figma navigation as an adapt benchmark only. Editor-entry and page-navigation work does not require a Figma board, Figma file, or migration artifact.

## 2026-08-31 Editor Navigation Delta

The current worktree reuses Layo's editor shell rather than adding a separate
Figma-style dashboard. The default Assets entry now exposes one real project
start action and labels built-in kits as static catalog examples. Help is a real
rail destination opened by click or `?`; global canvas shortcuts preserve text
entry and native control activation while layer rows retain intentional editor
selection shortcuts.
...
the 1024px fixed-shell constraint, focus return, and durable server/save/status
feedback remain separate editor-product gaps. See
...
- Browser editor shell with a generated Layo brand logo asset in the left mode rail and browser icon surfaces, a Figma-like left mode rail, default asset-library panel with thumbnail-rich starter kit cards, top file tabs, rulers, bottom floating toolbar, right Inspector action strip, right Inspector reusable-style search/filter/sort/grouping/duplicate management, expanded frame preset categories, creation, selection, dragging, corner-only resizing, direct double-click text editing, selection size badges, inspector geometry, grouped Inspector alignment/distribution controls, Inspector auto-layout normal/reverse direction/gap/row-gap/column-gap/padding/child-margin/child fill sizing/equal-cell grid auto-placement/manual grid cell placement/grid item span/cross-axis alignment/main-axis distribution/width-height fit sizing controls, viewport grid resize/add/remove/reorder controls including Ctrl/Command preserve-elements reorder, bounded span/named-area reorder, direct selected area/span boundary handles, and single-cell empty-cell area creation menu, viewport grid row/column header context menus including delete-track-with-shapes actions, grouped object context-menu selection/flip/fit/export/style actions with shortcut hints and matching cut/select/fit/style/rename/group/ungroup/alignment keyboard routes, color/text editing, undo/redo shortcuts, zoom, hover measurement overlays, selected-frame padding/child-spacing guides, a multi-selection group outline with combined dimensions, and file-version history with manual/automatic snapshots, recovery-safe server/Yjs restore, pinned checkpoints, manual delete, explicit retention cleanup, current-file change-summary diff, and a complete saved-document read-only canvas preview with named Exit/Restore banner and stale-request protection.
- Local image asset storage with browser clipboard paste, file drag/drop insertion for image nodes, context-menu image replacement, image fill/fit sizing modes, original image dimensions, context-menu original-size restore, and reference-aware cleanup for uploads cancelled before node attachment while retaining current-document and saved-version assets.
- No-write external migration preflight for Penpot ZIP/.penpot, Figma REST JSON, and opaque Figma binary files through server review, CLI, web API helpers, and the file panel, plus a first write-enabled Figma REST JSON import path for `CANVAS` pages, `FRAME`, `RECTANGLE`, and `TEXT` nodes through server storage, HTTP, web API helpers, and the file panel. Figma ZIP packages that include the REST JSON plus exported image assets now preserve rectangle image fills as Layo image nodes backed by local asset storage. Components, variants, effects, advanced style fidelity, and Penpot ZIP shape mapping still require dedicated mappers.
...
- Auto layout on frames/components with vertical, horizontal, and reverse flow.
- Cross-axis alignment and main-axis distribution for single-line Flex-like layout.
- Wrap, fallback gap, row/column gap split, padding, child item margins, child fill width/height sizing, equal-cell grid auto-placement, manual grid cell placement, grid item spans, static/absolute layout item positioning, min/max sizing rules, wrapped-line distribution, reverse row/column flow, viewport grid resize/add/remove/reorder controls including Ctrl/Command preserve-elements reorder, bounded span/named-area reorder, direct selected area/span boundary handles, single-cell empty-cell area creation menu, viewport grid row/column header context menus, viewport delete-track-with-shapes actions, and container fit width/height sizing.
...
- Add inspector controls for layout mode, flow direction including reverse row/column, wrap, alignment, distribution, fallback gap, row/column gap, padding, child margins, child fill sizing, grid rows/columns, manual child grid cell placement, child grid span, child layout-item positioning, container fit sizing, min/max sizing, and constraints. Add selected-grid viewport controls for direct row/column resize, append, delete, drag reorder, Ctrl/Command preserve-elements reorder, bounded span/named-area reorder, direct selected area/span boundary handles, single-cell empty-cell area creation menu, header-menu insert/duplicate/delete actions, and header-menu delete-track-with-shapes actions.
- Add Playwright coverage for automatic sibling repositioning and parent resize behavior.

Non-goals for the first slice:

- Multi-cell merge/split menus, baseline alignment, and advanced resizing semantics.
- Baseline alignment, percentage child sizing, and advanced intrinsic sizing rules.
...
- Figma REST/MCP import paths for real Figma files, where credentials are provided by the user. The first no-write preflight is in place, opaque `.fig` binaries still point users to REST export, Figma REST JSON can create a fresh Layo project for basic pages, frames, rectangles, and text, and Figma JSON packages with exported image assets can preserve rectangle image fills as Layo image nodes.
...
1. Keep the landed Lane 1 layout foundation, Flex alignment, reverse row/column flow, child-margin flow, child fill sizing, equal-cell grid auto-placement, manual grid cell placement, grid item span, static/absolute layout item positioning, wrap, row/column gap split, container fit sizing, min/max sizing, viewport grid resize/add/remove/reorder controls including Ctrl/Command preserve-elements reorder, bounded span/named-area reorder, direct selected area/span boundary handles, single-cell empty-cell area creation menu, viewport grid row/column header context menus, viewport delete-track-with-shapes actions, and PR #20 navigation fixes green.
...
7. Keep the Penpot/Figma migration preflight, Figma REST JSON write import, and Figma packaged image-fill import green, then extend full Figma/Penpot mapping on top of the local asset pipeline with components, variants, effects, advanced styles, and Penpot ZIP shape mapping.
