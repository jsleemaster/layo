...
| Version history | File-level version history, named versions, restore points. | Manual and automatic saved file versions exist with HTTP/MCP/UI list/save/read/restore, current-file diff preview, and pinned checkpoint controls. Full visual preview, retention/delete policy, and branchable review are not implemented. | Extend. Add visual preview, retention/delete controls, and branch/merge review on top of the landed history foundation. |
...
| Comments | Canvas comments, threads, replies, mentions, prototype comments. | Selected-node comments, replies, resolved state, viewport bubbles, persisted mention extraction, structured local team-member mention targets, local unread/read state, local project/file unread summaries with current-file mark-read, local viewer-targeted mention notification counts, latest-unread notification ordering, browser polling fallback, process-local SSE comment event delivery, and retained local create/reply/resolve activity feed exist. Prototype comments, full CRDT-backed live comment editing, cross-process/pubsub event fanout, and external notification channels are not implemented. | Adopt for collaboration. Comment data can stay local/team-owned. |
