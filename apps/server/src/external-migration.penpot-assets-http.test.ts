...
test("reviews imports and persists flattened Penpot stroke gradients", async () => {
  tempRoot = await mkdtemp(path.join(tmpdir(), "layo-"));
  const storage = new FileStorage(tempRoot);
  const server = createHttpServer(storage);
  const archive = createPenpotStrokeGradientExportArchive();
  ...
  expect(imported.json().imported).toMatchObject({
    ...
    mappedNodeCount: 2,
    skippedNodeCount: 0,
    project: { name: "Penpot Stroke Gradient Board" },
    file: { name: "Penpot Stroke Gradient Board", pages: [{ name: "Stroke gradients" }] }
  });

  const projects = await storage.listProjects();
  const persisted = await storage.readFile(projects[0].currentDocumentId);
  const frame = persisted.pages[0].children[0];
  expect(frame).toMatchObject({ id: `penpot-${frameId}`, kind: "frame", name: "Stroke gradient frame" });
  expect(frame.children[0]).toMatchObject({
