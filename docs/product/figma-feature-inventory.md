...
| Layer model | Pages, frames, shapes, text, images, groups, sections, parent/child hierarchy, selection. | Pages, frames, transparent groups, rectangles, text, images in type model, nested children, image insertion by paste/drop, layer ordering, node lock/visibility state, and context-menu group/ungroup for sibling selections. No sections UI. | Adopt. Add richer layer nesting controls, section-like organization, and more object context-menu groups. |
