...
| Layer model | Pages, frames, shapes, text, images, groups, sections, parent/child hierarchy, selection. | Pages, frames, rectangles, text, images in type model, nested children, and image insertion by paste/drop. No groups/sections UI. | Adopt. Add group/ungroup, layer reorder, and layer nesting. |
| Geometry | Move, resize, rotate, flip, x/y/w/h fields, bounding boxes. | Move, four corner resize handles, selection size badge, x/y/w/h fields, rotation in model only. | Adopt. Add edge handles, rotation, flip, proportional scale, and keyboard nudging. |
| Precision editing | Alignment, distribute, tidy, rulers, guides, layout guides. | Inspector alignment/distribution controls, hover measurement overlay, selected-frame padding and child-spacing guides. | Adopt after layout foundation. Add edge handles, rulers, manual guides, and tidy up later. |
...
| Fills and imagery | Solid fills, gradients, image fills, patterns, color picker. | Solid fill plus local image asset storage and image nodes inserted by clipboard paste or file drag/drop. | Adopt. Add gradients and image-fill controls next; patterns later. |
...
| Comments | Canvas comments, threads, replies, mentions, prototype comments. | Not implemented. | Adopt for collaboration. Comment data can stay local/team-owned. |
