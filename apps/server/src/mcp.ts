...
const nodeLayoutSchema = z.object({
  ...
  align_items: z.enum(["start", "center", "end", "stretch", "baseline", "last_baseline"]).default("start"),
...
const nodeLayoutItemSchema = z.object({
  ...
  align_self: z.enum(["start", "center", "end", "stretch", "baseline", "last_baseline"]).optional(),
  min_width: z.number().optional(),
  max_width: z.number().optional(),
  min_height: z.number().optional(),
  max_height: z.number().optional(),
...
const agentCommandSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("set_boolean_path_operation"),
    nodeId: z.string().min(1),
    operation: z.enum(["union", "difference", "intersection", "exclusion"])
  }),
  z.object({
    type: z.literal("detach_boolean_path"),
    nodeId: z.string().min(1)
  }),
  z.object({
