...
`2026-06-28-penpot-variant-area-layout-metadata.md` narrows the older full
variant-area gap: Layo now persists component variant-area layout metadata
across document contracts, Rust bindings, HTTP, MCP/agent command batches, and
the right Inspector. The remaining Penpot-comparable gap is direct canvas visual
variant-area editing and automatic reflow of variant source nodes.
