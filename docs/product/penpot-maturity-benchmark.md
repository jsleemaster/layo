...
2. Full comment workflow beyond selected-node create/list/resolve/reply, viewport unresolved-comment bubbles, persisted mentions, structured local team-member mention targets, local unread/read state, local project/file unread summaries, local viewer-targeted mention notification counts, browser-delivered local notification/activity refresh, and retained local create/reply/resolve activity feed: full live comment sync, websocket/SSE delivery, external notification channels, full visual diff, branch/review/merge, pinned/version preview controls, retention controls, and richer recovery workflow beyond the landed manual/automatic saved file versions, basic diff preview, and restore path.
