...
function replaceOnce(path, search, replacement) {
  ...
  if (!source.includes(search)) {
    throw new Error(`Missing expected block in ${path}: ${search.slice(0, 160)}`);
  }
  write(path, source.replace(search, replacement));
}

function insertAfter(path, search, addition) {
  replaceOnce(path, search, `${search}${addition}`);
}
...
replaceOnce(
  "apps/server/src/agent-control.ts",
  '  NodeLayoutItem,\n  TextWritingMode\n} from "./storage";',
  '  NodeLayoutItem,\n  TextOrientation,\n  TextWritingMode\n} from "./storage";'
);
replaceOnce(
...
replaceOnce(
  "crates/editor-core/bindings/NodeContent.ts",
  "font_family: string, writing_mode?: string | null, typography_token?: string | null, typography_style?: string | null,",
  "font_family: string, writing_mode?: string | null, text_orientation?: string | null, typography_token?: string | null, typography_style?: string | null,"
);
replaceOnce(
...
replaceOnce(
  "apps/web/src/editor-state.test.ts",
  '  test("baseline alignment synthesizes vertical writing mode text baselines", () => {\n    const oriented = executeEditorCommand',
...
replaceOnce(
  ...
  "canvas vertical text rendering for `vertical_rl`/`vertical_lr`, wrap,",
  "canvas vertical text rendering for `vertical_rl`/`vertical_lr`, explicit text-orientation controls for mixed/upright/sideways vertical glyph rendering, wrap,"
);
replaceOnce(
  "docs/product/penpot-maturity-benchmark.md",
  ...
  "deeper Unicode punctuation/script orientation fidelity, deeper CSS baseline group semantics, and remaining deeper resizing semantics remain immature."
);
replaceOnce(
  "docs/product/penpot-maturity-benchmark.md",
...
replaceOnce(
  "docs/product/penpot-maturity-benchmark.md",
  "slices: glyph-orientation controls beyond upright column glyphs, deeper CSS baseline group semantics, and remaining deeper resizing semantics.",
  "slices: deeper Unicode punctuation/script orientation fidelity, deeper CSS baseline group semantics, and remaining deeper resizing semantics."
);
