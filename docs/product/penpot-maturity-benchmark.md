...
fidelity, live theme/library subscription/update notifications, and hosted
permissioned registry sync. `2026-06-28-penpot-library-registry-sync.md` adds a
...
5. Full import/export workflow beyond the landed single-file, project-level,
   package-style shared-library Layo ZIP archive review UI, and server-local
   published library registry review/import UI/HTTP/MCP path: backup/restore
   runbooks, live shared-library subscription/update sync, hosted permissioned
   registry sync, and Penpot/Figma migration paths where feasible.
